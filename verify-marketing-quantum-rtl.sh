...
echo "=== Quantum-Level RTL Implementation Verification for Marketing Psychology Page ==="
...
css_refs=$(grep -c 'quantum-rtl-marketing.css' "$source_file")
js_refs=$(grep -c 'quantum-rtl.js' "$source_file")
marketing_js_refs=$(grep -c 'marketing-quantum-rtl.js' "$source_file")

echo "CSS references found: $css_refs"
echo "Quantum RTL JS references found: $js_refs"
echo "Marketing Quantum RTL JS references found: $marketing_js_refs"

if [ "$css_refs" -gt 0 ] && [ "$js_refs" -gt 0 ] && [ "$marketing_js_refs" -gt 0 ]; then
    ...
    if [ "$css_refs" -eq 0 ]; then
        echo "🔄 Adding quantum-rtl-marketing.css reference"
        perl -i -pe 's/(<\/head>)/<link rel="stylesheet" href="quantum-rtl-marketing.css">\n\1/' "$source_file"
    fi
    ...
    if [ "$marketing_js_refs" -eq 0 ]; then
        echo "🔄 Adding marketing-quantum-rtl.js reference"
        perl -i -pe 's/(<\/body>)/<script src="marketing-quantum-rtl.js"><\/script>\n\1/' "$source_file"
    fi
fi

echo "\n5. Verifying quantum-rtl-zone class in article..."
zone_class=$(grep -c 'quantum-rtl-zone' "$source_file")
if [ "$zone_class" -gt 0 ]; then
    echo "✅ quantum-rtl-zone class found $zone_class times"
else
    echo "⚠️  Adding quantum-rtl-zone class to article"
    perl -i -pe 's/<article class="prose/<article class="prose quantum-rtl-zone/' "$source_file"
fi
...
rtl_css_props=$(grep -c -E 'direction: rtl|text-align: right|unicode-bidi|padding-right|padding-left' quantum-rtl-marketing.css)
echo "RTL CSS properties found: $rtl_css_props"
if [ "$rtl_css_props" -ge 10 ]; then
    echo "✅ Sufficient RTL CSS properties in quantum-rtl-marketing.css"
else
...
marketing_class=$(grep -c 'class MarketingQuantumRTL' marketing-quantum-rtl.js)

echo "QuantumRTLObserver class: $quantum_class"
echo "MarketingQuantumRTL class: $marketing_class"

if [ "$quantum_class" -eq 1 ] && [ "$marketing_class" -eq 1 ]; then
...
echo "============================="
echo ""
echo "Recommendation: Run 'final-verification-and-deploy-marketing.sh' to complete the implementation"
echo "Then deploy to Firebase with 'firebase deploy --only hosting:captivate-audiences-marketing-psychology.html'"